...
**Def**. Diferencijabilno preslikavanje $\mathbf{r}:I\to\mathbb{R}^{3}$ zove se **kriva** u $\mathbb{R}^{3}$. $\mathbf{r}(t)=\big(x(t),\,y(t),\,z(t)\big)$.
Kriva je **regularna** ako  $\forall t\in I\quad\big(x'(t),\,y'(t),\,z'(t)\big)\ne \mathbf{0}$.

**Def**. $\tau$ je **tangenta** na (regularnu) krivu $\mathbf{r}$ u tački $\mathbf{a}=\mathbf{r}(t_{0})$, ako prolazi kroz $\mathbf{a}$ i vektor pravca joj  je $\mathbf{r}'(t_{0})=\big(x'(t),\,y'(t),\,z'(t)\big)$
